...
bool BMP280::initialize(uint8_t a, uint8_t chipid) {
	_i2caddr = a;
...
{
	bool r = I2Cdev::writeByte(_i2caddr, reg, value);
}
...
{
	uint8_t value =  0;
	bool r = I2Cdev::readByte(_i2caddr, reg, &value);
	return value;
}
...
{
	uint16_t value;
	bool r = I2Cdev::readWord(_i2caddr, reg, &value);
	return value;
}
...
uint32_t BMP280::read24(uint8_t reg)
{
	uint32_t value;
	uint8_t data[3];
	bool r = I2Cdev::readBytes(_i2caddr, reg, 3, data);
	value = data[0];
	value <<= 8;
	value |= data[1];
	value <<= 8;
